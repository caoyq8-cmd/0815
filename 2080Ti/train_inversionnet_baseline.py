...
class TrainConfig:
    ...
    output_dir: str = "./results_inversionnet_baseline"
...
# InversionNet 风格模型
# 说明：
# 这不是你原项目里“官方精确 InversionNet”复刻，
# 而是一个 inversion-style encoder-decoder baseline：
# 强编码、无 U-Net 式长跳连、瓶颈残差块、逐级上采样恢复。
# 很适合和当前 UNet 主基线做公平结构对比。
# =========================
class ConvBNAct(nn.Module):
    def __init__(self, in_ch, out_ch, k=3, s=1, p=1, act="relu", dropout=0.0):
        super().__init__()
        layers = [
...
class ResidualBlock(nn.Module):
    def __init__(self, ch, act="relu", dropout=0.0):
        super().__init__()
        self.conv1 = ConvBNAct(ch, ch, k=3, s=1, p=1, act=act, dropout=dropout)
...
class UpBlock(nn.Module):
    def __init__(self, in_ch, out_ch, act="relu", dropout=0.0):
        super().__init__()
        self.up = nn.ConvTranspose2d(in_ch, out_ch, kernel_size=2, stride=2)
        self.refine = nn.Sequential(
            ConvBNAct(out_ch, out_ch, k=3, s=1, p=1, act=act, dropout=dropout),
            ConvBNAct(out_ch, out_ch, k=3, s=1, p=1, act=act, dropout=dropout),
        )
...
class InversionNetBaseline(nn.Module):
    def __init__(self, in_channels=2, out_channels=1, base_ch=32, bottleneck_blocks=4, dropout=0.0):
        super().__init__()

        # encoder: 256 -> 128 -> 64 -> 32 -> 16 -> 8
        self.stem = nn.Sequential(
            ConvBNAct(in_channels, base_ch, k=5, s=1, p=2, act="relu", dropout=dropout),
            ConvBNAct(base_ch, base_ch, k=3, s=1, p=1, act="relu", dropout=dropout),
        )

        self.enc1 = nn.Sequential(
            ConvBNAct(base_ch, base_ch * 2, k=4, s=2, p=1, act="relu", dropout=dropout),   # 128
            ConvBNAct(base_ch * 2, base_ch * 2, k=3, s=1, p=1, act="relu", dropout=dropout),
        )
        self.enc2 = nn.Sequential(
            ConvBNAct(base_ch * 2, base_ch * 4, k=4, s=2, p=1, act="relu", dropout=dropout), # 64
            ConvBNAct(base_ch * 4, base_ch * 4, k=3, s=1, p=1, act="relu", dropout=dropout),
        )
        self.enc3 = nn.Sequential(
            ConvBNAct(base_ch * 4, base_ch * 8, k=4, s=2, p=1, act="relu", dropout=dropout), # 32
            ConvBNAct(base_ch * 8, base_ch * 8, k=3, s=1, p=1, act="relu", dropout=dropout),
        )
        self.enc4 = nn.Sequential(
            ConvBNAct(base_ch * 8, base_ch * 8, k=4, s=2, p=1, act="relu", dropout=dropout), # 16
            ConvBNAct(base_ch * 8, base_ch * 8, k=3, s=1, p=1, act="relu", dropout=dropout),
        )
        self.enc5 = nn.Sequential(
            ConvBNAct(base_ch * 8, base_ch * 8, k=4, s=2, p=1, act="relu", dropout=dropout), # 8
            ConvBNAct(base_ch * 8, base_ch * 8, k=3, s=1, p=1, act="relu", dropout=dropout),
        )

        blocks = []
        for _ in range(bottleneck_blocks):
            blocks.append(ResidualBlock(base_ch * 8, act="relu", dropout=dropout))
        self.bottleneck = nn.Sequential(*blocks)

        # decoder: 8 -> 16 -> 32 -> 64 -> 128 -> 256
        self.dec5 = UpBlock(base_ch * 8, base_ch * 8, act="relu", dropout=dropout)
        self.dec4 = UpBlock(base_ch * 8, base_ch * 8, act="relu", dropout=dropout)
        self.dec3 = UpBlock(base_ch * 8, base_ch * 4, act="relu", dropout=dropout)
        self.dec2 = UpBlock(base_ch * 4, base_ch * 2, act="relu", dropout=dropout)
        self.dec1 = UpBlock(base_ch * 2, base_ch, act="relu", dropout=dropout)
