...
def hybrid_step(
    ...
    neural_loss_before, grad, neural_pred_before = compute_neural_grad(
        model,
        current,
        target_dobs,
        mean_real,
    ...
    for cand in candidates:
        cand_480 = resize_np(cand["speed"], 480)

        true_pred = forward_cbs_dobs(
            cand_480,
            src_indices,
            rec_indices,
            args,
        ...
        true_abs = abs_loss_complex(true_pred, target_dobs)
        true_l1 = rel_l1_complex(true_pred, target_dobs)
        true_l2 = rel_l2_complex(true_pred, target_dobs)

        item = {
            "candidate": cand["name"],
            "step_used": cand["step_used"],
            "sign": cand["sign"],
            "true_cbs_abs_loss": true_abs,
            "true_cbs_rel_l1": true_l1,
            "true_cbs_rel_l2": true_l2,
        }
        all_candidates.append(item)

        if best is None or true_abs < best["true_cbs_abs_loss"]:
            best = {
                **item,
                "speed": cand["speed"],
                "true_pred": true_pred,
    ...
    neural_loss_after, neural_pred_after = eval_neural_np(
        model,
        chosen,
        target_dobs,
        mean_real,
    ...
    info = {
        "neural_loss_before": neural_loss_before,
        "neural_loss_after": neural_loss_after,
        "chosen_candidate": best["candidate"],
        "chosen_sign": best["sign"],
        "chosen_step_used": best["step_used"],
        "true_cbs_abs_loss": best["true_cbs_abs_loss"],
        "true_cbs_rel_l1": best["true_cbs_rel_l1"],
        "true_cbs_rel_l2": best["true_cbs_rel_l2"],
        "grad_rms": grad_rms,
        "prior_rms": prior_rms,
        "update_abs_mean": float(np.mean(np.abs(chosen - current))),
        "update_abs_max": float(np.max(np.abs(chosen - current))),
        "neural_dobs_rel_l1": rel_l1_complex(neural_pred_after, target_dobs),
        "neural_dobs_rel_l2": rel_l2_complex(neural_pred_after, target_dobs),
        "neural_dobs_abs_loss": abs_loss_complex(neural_pred_after, target_dobs),
        "all_candidates": all_candidates,
    }

    return chosen, info, best["true_pred"], neural_pred_after
...
def main():
    ...
    init_neural_loss, init_neural_pred = eval_neural_np(
        model,
        current,
        target_dobs,
        mean_real,
    ...
    init_true_pred = forward_cbs_dobs(
        current_480,
        src_indices,
        rec_indices,
        args,
    ...
    init_row = {
        ...
        "neural_loss": init_neural_loss,
        "neural_dobs_rel_l1": rel_l1_complex(init_neural_pred, target_dobs),
        "neural_dobs_rel_l2": rel_l2_complex(init_neural_pred, target_dobs),
        "neural_dobs_abs_loss": abs_loss_complex(init_neural_pred, target_dobs),
        "true_cbs_rel_l1": rel_l1_complex(init_true_pred, target_dobs),
        "true_cbs_rel_l2": rel_l2_complex(init_true_pred, target_dobs),
        "true_cbs_abs_loss": abs_loss_complex(init_true_pred, target_dobs),
    }
    ...
    for it in range(1, args.num_iters + 1):
        print("=" * 80)
        print(f"[Hybrid Iteration {it}/{args.num_iters}]")
        print("=" * 80)

        current, info, true_pred, neural_pred = hybrid_step(
            current,
            condition_fixed,
            target_dobs,
            src_indices,
        ...
        np.savez_compressed(
            ...
            true_cbs_pred_dobs=true_pred.astype(np.complex64),
            neural_pred_dobs=neural_pred.astype(np.complex64),
        )
