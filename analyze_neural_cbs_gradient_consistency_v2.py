...
def compute_cbs_grad_and_forward(
    ...
    loss_mse = complex_mse(dobs_pred, target_dobs)
    ...
    return (
        grad_raw,
        grad_smooth,
        float(reported_loss.detach().cpu()),
        loss_abs,
        loss_mse,
        dobs_pred,
    )
...
def main():
    ...
    for base_i, (base, records) in enumerate(groups.items(), start=1):
        ...
        for alpha, path in records:
            ...
            g_cbs_raw_480, g_cbs_smooth_480, reported_loss, true_abs_loss, true_mse_loss, true_forward = (
            ...
            row = {
                ...
                "true_cbs_mse_loss_to_gt": true_mse_loss,
                "cbs_reported_abs_loss": reported_loss,
            ...
            if args.descent_test:
                ...
                # IMPORTANT: use true CBS complex MSE here because both gradients
                # are intended to represent an L2/MSE-type data-fidelity objective.
                best_nn_mse = true_mse_loss
                best_nn_abs = true_abs_loss
                best_nn_factor = 0.0

                best_cbs_mse = true_mse_loss
                best_cbs_abs = true_abs_loss
                best_cbs_factor = 0.0
                ...
                for fac in args.descent_factors:
                    ...
                    loss_nn_mse = complex_mse(pred_nn, target_dobs)
                    loss_nn_abs = complex_mae(pred_nn, target_dobs)
                    if loss_nn_mse < best_nn_mse:
                        best_nn_mse = loss_nn_mse
                        best_nn_abs = loss_nn_abs
                    ...
                    loss_cbs_mse = complex_mse(pred_cbs, target_dobs)
                    loss_cbs_abs = complex_mae(pred_cbs, target_dobs)
                    if loss_cbs_mse < best_cbs_mse:
                        best_cbs_mse = loss_cbs_mse
                        best_cbs_abs = loss_cbs_abs
                        best_cbs_factor = float(fac)

                row.update({
                    "nn_descent_success_l2": int(best_nn_mse < true_mse_loss - 1e-16),
                    "nn_best_true_cbs_mse_loss": float(best_nn_mse),
                    "nn_best_true_cbs_abs_loss_at_l2_best": float(best_nn_abs),
                    "nn_best_true_cbs_l2_rel_reduction": float(
                        (true_mse_loss - best_nn_mse) / (true_mse_loss + 1e-20)
                    ),
                    "nn_best_factor_l2": best_nn_factor,

                    "cbs_descent_success_l2": int(best_cbs_mse < true_mse_loss - 1e-16),
                    "cbs_best_true_cbs_mse_loss": float(best_cbs_mse),
                    "cbs_best_true_cbs_abs_loss_at_l2_best": float(best_cbs_abs),
                    "cbs_best_true_cbs_l2_rel_reduction": float(
                        (true_mse_loss - best_cbs_mse) / (true_mse_loss + 1e-20)
                    ),
                    "cbs_best_factor_l2": best_cbs_factor,
                })
            ...
            print(
                ...
                + (
                    f" | nn_desc_L2={row['nn_descent_success_l2']} "
                    f"cbs_desc_L2={row['cbs_descent_success_l2']}"
    ...
    for alpha in sorted(by_alpha.keys()):
        ...
        if args.descent_test:
            s["nn_descent_success_rate_l2"] = float(
                np.mean([r["nn_descent_success_l2"] for r in rr])
            )
            s["cbs_descent_success_rate_l2"] = float(
                np.mean([r["cbs_descent_success_l2"] for r in rr])
            )
            s["nn_best_true_cbs_l2_rel_reduction_mean"] = float(
                np.mean([r["nn_best_true_cbs_l2_rel_reduction"] for r in rr])
            )
            s["cbs_best_true_cbs_l2_rel_reduction_mean"] = float(
                np.mean([r["cbs_best_true_cbs_l2_rel_reduction"] for r in rr])
            )
    ...
    overall = {
        "num_rows": len(rows),
        "num_base": len(groups),
        "note_gradient_magnitude": (
            "Raw NN/CBS RMS ratios are scale-dependent because CBS adjoint and neural MSE "
            "use different normalization/constants. Use cosine and normalized descent tests "
            "as the primary gradient-quality evidence."
        ),
    ...
    if args.descent_test and rows:
        overall["nn_descent_success_rate_l2"] = float(
            np.mean([r["nn_descent_success_l2"] for r in rows])
        )
        overall["cbs_descent_success_rate_l2"] = float(
            np.mean([r["cbs_descent_success_l2"] for r in rows])
        )
