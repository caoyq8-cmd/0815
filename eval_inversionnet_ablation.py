...
def normalize_to_unit(x: torch.Tensor, vmin: float, vmax: float) -> torch.Tensor:
    x = (x - vmin) / (vmax - vmin)
...
def compute_ssim(
    ...
    """
    稳定版 SSIM:
    1. 先把物理量纲图像按固定范围映射到 [0,1]
    2. 再按标准 SSIM 公式计算
    """
    pred = normalize_to_unit(pred, value_min, value_max)
    target = normalize_to_unit(target, value_min, value_max)
    ...
    window = gaussian_window(window_size, sigma, channels, device=device, dtype=pred.dtype)
    ...
    ssim_val = float(ssim_map.mean().item())
...
def main():
    ...
    with torch.no_grad():
        for local_idx, (x, y) in enumerate(test_loader):
            ...
            ssim = compute_ssim(pred, y, value_min=target_min, value_max=target_max)
