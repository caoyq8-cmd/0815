...
class TrainConfig:
    ...
    output_dir: str = "./ablation_runs/hf_refiner"
...
def lowpass(img: torch.Tensor, kernel_size: int = 9) -> torch.Tensor:
    return F.avg_pool2d(img, kernel_size=kernel_size, stride=1, padding=kernel_size // 2)


def highpass(img: torch.Tensor, kernel_size: int = 9) -> torch.Tensor:
    return img - lowpass(img, kernel_size=kernel_size)
...
class HFLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        pred_hf = highpass(pred, kernel_size=self.kernel_size)
        target_hf = highpass(target, kernel_size=self.kernel_size)
        return F.l1_loss(pred_hf, target_hf)


class CompositeLoss(nn.Module):
    def __init__(self, lambda_l1=1.0, lambda_mse=0.2, lambda_grad=0.1, lambda_hf=1.0, hf_kernel_size=9):
        super().__init__()
        ...
        self.hf_loss = HFLoss(kernel_size=hf_kernel_size)
...
class TwoStageHFModel(nn.Module):
...
def save_visualizations(model: nn.Module, loader: DataLoader, cfg: TrainConfig, epoch: int, device: str):
    ...
    inv_hf = highpass(inv_pred, kernel_size=cfg.hf_kernel_size).cpu()
    final_hf = highpass(final_pred, kernel_size=cfg.hf_kernel_size).cpu()
    gt_hf = highpass(gt, kernel_size=cfg.hf_kernel_size).cpu()

    n = min(cfg.max_vis_samples, x.shape[0])
    for i in range(n):
        fig, axes = plt.subplots(2, 4, figsize=(16, 7))

        axes[0, 0].imshow(x_cpu[i, 0], cmap="gray")
        axes[0, 0].set_title("input ch0")
        axes[0, 1].imshow(gt[i, 0], cmap="magma")
        axes[0, 1].set_title("gt")
        axes[0, 2].imshow(inv_pred[i, 0], cmap="magma")
        axes[0, 2].set_title("inv pred")
        axes[0, 3].imshow(final_pred[i, 0], cmap="magma")
        axes[0, 3].set_title("final pred")

        axes[1, 0].imshow(x_cpu[i, 1], cmap="gray")
        axes[1, 0].set_title("input ch1")
        axes[1, 1].imshow(gt_hf[i, 0], cmap="bwr")
        axes[1, 1].set_title("gt highpass")
        axes[1, 2].imshow(inv_hf[i, 0], cmap="bwr")
        axes[1, 2].set_title("inv highpass")
        axes[1, 3].imshow(final_hf[i, 0], cmap="bwr")
...
def save_curves(history: Dict[str, list], outdir: str):
    ...
    _plot(["val_psnr_final", "val_psnr_inv"], "Val PSNR", "curve_val_psnr.png")
    _plot(["val_hf_l1_final", "val_hf_l1_inv"], "High-Frequency L1", "curve_hf_l1.png")
...
def evaluate(model, loader, criterion, cfg, device):
    ...
    for x, y in loader:
        ...
        hf_inv = F.l1_loss(highpass(inv_pred, cfg.hf_kernel_size), highpass(y, cfg.hf_kernel_size)).item()
        hf_final = F.l1_loss(highpass(final_pred, cfg.hf_kernel_size), highpass(y, cfg.hf_kernel_size)).item()
    ...
    return {
        ...
        "hf_l1_inv": total_hf_inv / max(n_batches, 1),
        "hf_l1_final": total_hf_final / max(n_batches, 1),
    }
...
def parse_args():
    ...
    parser.add_argument("--lambda_hf", type=float, default=1.0)

    parser.add_argument("--hf_kernel_size", type=int, default=9)
...
def main():
    ...
    cfg = TrainConfig(
        ...
        hf_kernel_size=args.hf_kernel_size,
    ...
    model = TwoStageHFModel(inversion_model, refiner_model).to(cfg.device)
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        history["val_hf_l1_inv"].append(val_metrics["hf_l1_inv"])
        history["val_hf_l1_final"].append(val_metrics["hf_l1_final"])
        ...
        print(
            ...
            f"inv_hf={val_metrics['hf_l1_inv']:.6f} | "
            f"final_hf={val_metrics['hf_l1_final']:.6f}"
        )
